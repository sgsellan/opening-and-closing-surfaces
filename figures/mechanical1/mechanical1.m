...
[V,F] = read_triangle_mesh('../../data/mechanical1.obj'); % read input
V = V-min(min(V));
V = V./(max(max(V)));
h = 0.01;
bd = 1/.07;
...
writeOBJ('mechanical1_input.obj',V,F);
[U,G] = closing_flow(V,F,'Bound',bd,'EdgeLength',h,'TimeStep',dt,...
    'MaxIter',100,'RemeshIterations',1,'Debug',false,'Plot',true,'Write',false);
writeOBJ('mechanical1_output.obj',U,G);
...
[Vgt,Fgt] = readOBJ('mechanical1_input.obj');
[V,F] = read_triangle_mesh('mechanical1_output.obj');
...
drawnow
%pause
writeOBJ('mechanical1_active.obj',v_active,f_active);
writeOBJ('mechanical1_inactive.obj',v_inactive,f_inactive);